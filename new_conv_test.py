...
class TestSparseConv(unittest.TestCase):
    ...
    def test_conv3d(self):
        ...
        with _test_eager_guard():
            ...
            p_input = sparse.sparse_coo_tensor(p_indices, p_values, p_shape)

            p_input = sparse.coalesce(p_input)
            p_conv = sparse.nn.Conv3D(in_channels=config[i]['in_channels'], out_channels=config[i]['out_channels'], kernel_size=config[i]['kernel_size'],
